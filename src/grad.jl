...
function _add_variables_grad!(prb::Problem)
    ...
    @variable(Lower(model), c[1:cache.N], Bin)
...
function _add_constraints_grad!(prb::Problem)
    ...
    a = cache.g
    b = [cache.f[n] - a[n]*data.grid[n] for n in 1:cache.N]
    if data.force_prop == convex
        @constraint(Lower(model), planes[n = 1:cache.N, i = 1:cache.N], y[n] >= (a[i]*data.grid[n]+b[i])*c[i])
    elseif data.force_prop == concave
        @constraint(Lower(model), planes[n = 1:cache.N, i = 1:cache.N], y[n] <= (a[i]*data.grid[n]+b[i])*c[i])
    end
    @constraint(Lower(model), choose[n = 1:cache.N], sum(c) == data.pieces)
    if data.norm == L1
        error = model[:error]
